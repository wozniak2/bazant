# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
#                analiza treści ofert pracy dla obszaru finansów i rachunkowości
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------


# unifikujemy bazy z obu portali
offers.p.finan <- aggregate(word ~ offer, data = offers.p.finan, FUN = paste, collapse = " ")
offers.finan.ip <- aggregate(word ~ offer, data = offers.finan.ip, FUN = paste, collapse = " ")

# preprocessing + łączenie baz w jedną całość
require(dplyr)
offers <- bind_rows(offers.p.finan, offers.finan.ip)
...
write.csv(ekon_tf_idf, "finan_tf_idf.csv", fileEncoding = "UTF-8")
