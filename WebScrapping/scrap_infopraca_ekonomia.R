...
url1 <- "&ct=administracja-publiczna&ct=administracja-biurowa&ct=biuro-dokumentacja&ct=doradztwo-konsulting&ct=franczyza-wlasny-biznes"
...
write.csv(page.ekon.ip, "page_ekon_ip.csv")
...
require(tcltk)
pb <- tkProgressBar(title = "progress bar", min = 0,
                    max = length(page.ekon.ip$link), width = 300)
url2 <- "https://infopraca.pl"
map_df(1:length(page.ekon.ip$link), function(i){
  ...
  try(print(page.ekon.ip$link[i]))
  print(i)
  setTkProgressBar(pb, i, label=paste( round(i/length(page.ekon.ip$link)*100, 0),
                                       "% done"))
  
  pp <- try(read_html(paste0(url2, page.ekon.ip$link[i])))
  if (inherits(pp, 'try-error')) {read_html(paste0(url2, page.ekon.ip$link[i-1]))} 
  
  tryCatch(data.frame(word = html_text(html_nodes(pp,".description")), offer=page.ekon.ip$title[i]), error=function(e) data.frame())
...
data.frame(word = html_text(html_nodes(ppi,".description")))

html_nodes(ppi, "#iframeId1")  %>% 
  html_attr("src")



pb <- tkProgressBar(title = "progress bar", min = 0,
                    max = length(page$link), width = 300)
...
map_df(1:length(page.ekon.ip$link), function(i){
  ...
  try(print(page$link[i]))
  print(i)
  setTkProgressBar(pb, i, label=paste( round(i/length(page$link)*100, 0),
                                       "% done"))
  
  pp <- try(read_html('https://www.infopraca.pl', paste0(as.character(page$link[i]))))
  if (inherits(pp, 'try-error')) {read_html(paste0(as.character(page$link[i-1])))} 
  
  tryCatch(data.frame(word = html_text(html_nodes(pp,"#iframeId1"))), error=function(e) data.frame())
