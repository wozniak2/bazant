...
url <- "https://www.pracuj.pl/praca?rd=30&cc=5001%2c5037%2c5008%2c5023%2c5027&pn="
...
write.csv(page.p, "pagep.csv")


#  clear the missing data
page.p <- page.p[complete.cases(page.p), ]



# wczytujemy biblioteke 'dplyr' i konwerujemy dane do tabeli tibble; konwertujemy dane na tekst
library(dplyr)
title.p_df <- as_tibble(page.p$title)
title.p_df <- mutate(title.p_df, text = as.character(page.p$title))
...
require(tcltk)
pb <- tkProgressBar(title = "progress bar", min = 0,
                    max = length(page.p$link), width = 300)

map_df(1:length(page.p$link), function(i){
  
    if((i %% 50) == 0){
      message("taking a break")
      Sys.sleep(15 + sample(3:10, 1))
    }
  
  tryCatch({(print(page.p$link[i]))}, 
           error=function(e) print("Catch Error"))
  print(i)
  setTkProgressBar(pb, i, label=paste( round(i/length(page.p$link)*100, 0),
                                       "% done"))
  
  pp <- tryCatch({
          read_html(paste0(as.character(page.p$link[i])))}, 
            error=function(e) read_html(paste0(as.character(page.p$link[1]))))
  
  tryCatch({
    data.frame(word = html_text(html_nodes(pp,".OfferView2JLZCu")),
               offer = page.p$title[i])}, error=function(e) data.frame())
...
offers.p <- rbind(offers.p, offers.p1)

setwd("C:/Users/wozni/Google Drive/UAM/PowerBażant/scripts/")
write.csv(offers.p, "offers_p_ekon.csv")
