...
url <- "https://www.pracuj.pl/praca?rd=30&cc=5002%2c5013%2c5015%2c5016&pn="
...
write.csv(page.p.info, "pagep_info.csv")

#  clear the missing data
page.p.info <- page.p.info[complete.cases(page.p.info), ]
...
require(tcltk)
pb <- tkProgressBar(title = "progress bar", min = 0,
                    max = length(page.p.info$link), width = 300)

map_df(1:length(page.p.info$link), function(i){
  ...
  tryCatch({(print(page.p.info$link[i]))}, 
           error=function(e) print("Catch Error"))
  print(i)
  setTkProgressBar(pb, i, label=paste( round(i/length(page.p.info$link)*100, 0),
                                       "% done"))
  
  pp <- tryCatch({
          read_html(paste0(as.character(page.p.info$link[i])))}, 
            error=function(e) read_html(paste0(as.character(page.p.info$link[1]))))
  
  tryCatch({
    data.frame(word = html_text(html_nodes(pp,".OfferView2JLZCu")),
               offer = page.p.info$title[i])}, error=function(e) data.frame())
...
offers.p.info <- rbind(offers.p.info, offers.p1.info)

setwd("C:/Users/wozni/Google Drive/UAM/PowerBażant/scripts/")
write.csv(offers.p.info, "offers_p_info.csv")
